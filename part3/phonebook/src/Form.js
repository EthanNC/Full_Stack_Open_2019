...
const Form = ({ persons, setPersons }) => {
  ...
  async function addToDb(newObject){
    const headers = {
      'Content-Type': 'application/json',
    }
    try {
      await axios.post(`http://localhost:3001/api/persons/`, newObject, headers)
    }
    catch (error) {
      console.log(error)
    }
  ...
  async function updateDb(newObject) {
    ...
    //TODO:added object ID to put request
    try {
      await axios.put(`http://localhost:3001/api/persons/${newObject.id}`, newObject, headers)
    }
    catch (error) {
      console.log(error)
    }
